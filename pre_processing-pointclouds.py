...
# CHANGE THESE PATHS:
INPATH = 'PASCAL3D+_release1.1/CAD/'
OUTPATH = 'pointclouds/'
SOURCE = 'sampling.mlx'
MESHLAB_PATH = '/Applications/meshlab.app/Contents/MacOS/meshlabserver'


#       THE FOLLOWING FUNCTIONS TO MANIPULATE PLY FILES WERE WRITTEN BY:
...
# paths...
NEW_OUTPATH = OUTPATH
OUTPATH = 'sampled_meshes/'
classes = ['aeroplane', 'bicycle', 'boat', 'bottle', 'bus', 'car', 'chair', 'diningtable', 'motorbike', 'sofa', 'train', 'tvmonitor']


# use meshlab to sample meshes to get pointclouds (saved as .ply)
# Poisson Disk sampling is used
for cls in classes:
    files = os.listdir(INPATH + cls)
    for fn in files:
        print(fn)
        inpath = INPATH + cls + '/' + fn
        outpath = OUTPATH + cls + fn.strip('.off') + '.ply'
        print('\n', inpath)
        print('\n', outpath)
        print('\n')
        os.system(MESHLAB_PATH + ' -i ' + inpath + ' -o '+outpath+' -s ' + SOURCE)

numbers_replace = {'01': str(0),
                   '02': str(1),
                   '03': str(2),
                   '04': str(3),
                   '05': str(4),
...
# use the above defined functions to convert the .ply files to numpy arrays
files = os.listdir(OUTPATH)
for fn in files:
    if '.ply' not in fn:
        continue
    print(fn)
    np.save(NEW_OUTPATH + fn[0:-6] + numbers_replace[fn[-6:-4]], np.array(read_ply(OUTPATH + fn)['points']))
